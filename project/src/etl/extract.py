...
def extract_all(data_dir="data/raw/"):
    ...
    for name, path in files.items():
        ...
        if name == 'customers':
            ...
            df = read_xlsx(path, sheet_name='products')  # для xlsx указываем лист
        elif name == 'payments':
            df = read_csv(path, sep='^')  # передаём разделитель
        elif name == 'events':
            df = read_xml(path, xpath='.//event')  # для XML указываем путь
        else:
            continue
