...
def standardize_columns(df):
    """Приводит названия колонок к нижнему регистру, заменяет пробелы на подчёркивания."""
    df.columns = [col.lower().strip().replace(' ', '_') for col in df.columns]
    return df

def safe_convert_datetime_old(series):
    """Конвертирует в datetime, некорректные значения -> NaT."""
...
def safe_convert_numeric(series):
    """Конвертирует в число, заменяя 'error_amount', 'N/A', 'NA', '' на NaN."""
...
def clean_id(value):
    ...
    Если value – число (int или float), преобразует его в int и затем в str.
    Если value – строка, обрезает пробелы и возвращает как есть.
    Если value – None или NaN, возвращает pd.NA.
    """
    if pd.isna(value):
        return pd.NA
    try:
        # Пытаемся преобразовать в float, затем в int (чтобы убрать .0), затем в строку
        return str(int(float(value)))
    except (ValueError, TypeError):
        # Если не число, просто возвращаем строку без лишних пробелов
