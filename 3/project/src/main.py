...
def run_etl():
    engine = get_engine()
    logging.info("\033[94mНачало ETL\033[0m")
    
    # Извлечение
    raw = extract_all("data/raw/")
    if not raw:
        logging.error("\033[91mНет данных для загрузки\033[0m")
        return
    ...
    # Проверка
    logging.info("\033[94mЗапуск проверки качества данных\033[0m")
    all_errors = []
    ...
    logging.info("\033[92mETL завершён\033[0m")
